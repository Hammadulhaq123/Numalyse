...
const Login = () => {
    return (
        <div id='login-container' className='fixed transition-all rounded-b-[30px] duration-200 -translate-y-[25vh] xl:-translate-y-[13vh] top-0 left-0 h-[25vh] xl:h-[12vh] w-screen bg-gray-200'>
            <div className="w-full h-full flex flex-col xl:flex-row items-center gap-4 xl:gap-2 justify-start p-4">
                ...
                <button className="w-full xl:w-[10%] h-14 rounded-full outline-none focus:outline-none  bg-[#04aa6d] text-white px-4 text-xl hover:opacity-90">Login</button>
                ...
                <button onClick={closeLogin} className="w-full xl:w-10 bg-[#2e2e2e] h-16 xl:absolute bottom-2 right-4  text-[#fff] tex-3xl font-extrabold flex gap-2 items-center justify-center rounded-full">
                    <span className="xl:hidden text-xl block">
                        close
                    </span>
                    <AiOutlineArrowUp className="text-3xl xl:text-xl" />
                </button>
            </div>
        </div>
    )
